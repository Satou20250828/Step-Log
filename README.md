...
### フロントエンド
- **使用技術**：HTML / Tailwind CSS / TypeScript

- **選定理由**：  
  HTMLを基盤とし、画面構造を明確に保ったままUIを構築するためです。  
  Tailwind CSSを用いることで、クラス設計を通してUIの意図がコード上で把握しやすくなり、  
  実装とデザインの対応関係を明確に保てると考えました。  
  また、TypeScriptを使用することで、データの受け渡しや状態管理を型として明示し、  
  実装段階での不整合や想定外の挙動を防ぐことを重視しています。

### バックエンド
- **使用技術**：Ruby / Ruby on Rails

- **選定理由**：  
  CRUD機能を少ないコード量で実装できる点に強みがあるため採用しました。  
  また、「1ユーザーにつき1習慣」「1日1記録」といった制約のある業務ロジックを、  
  モデルやバリデーションとして明確に表現しやすく、  
  仕様とコードの対応関係を保った設計がしやすいと考えました。
...
- **選定理由**：  
  Railsとの親和性が高く、  
  UNIQUE制約や外部キー制約などをデータベースレベルで定義できるため採用しました。  
  アプリケーション側の実装だけに依存せず、  
  データの整合性を構造として担保する設計を重視しています。

### 認証・ユーザー管理
- **使用技術**：Cookie / LocalStorage

- **選定理由**：  
  ログイン機能を実装せず、初回アクセスからすぐに利用できる体験を提供するためです。  
  そのため、サーバー側でのユーザー管理は行わず、  
  最小限の状態管理をクライアント側で完結させる設計としました。

### デプロイ・実行環境
- **使用技術**：Vercel（フロントエンド） / Ruby on Rails（バックエンド）

- **選定理由**：  
  フロントエンドは、ビルドから公開までの流れが簡潔で、  
  実装した内容を迅速に確認・共有できる点を重視して選定しました。  
  バックエンドは、開発環境と本番環境の差異を極力小さくし、  
  Railsの構成を維持したまま運用できることを意識しています。

本アプリでは、新しい技術を多く採用することよりも、  
課題設定と設計意図を明確にし、サービスを最後まで完成・公開することを重視しています。
