...
- **Render / Neon PostgreSQL**
  - 選定理由: 設定の手間を最小限に抑え、スムーズにインターネットへ公開できる構成として想定。運用コストを考え無料枠で収まるようにしかったため。
...
## 🚀 Render 本番運用メモ（初回起動の遅延対策）

- **原因1: Web 起動時の `db:prepare` 実行**
  - コンテナ起動ごとに DB 接続・マイグレーション確認が走ると、コールドスタートが遅くなります。
  - 本リポジトリでは `bin/docker-entrypoint` を修正し、production では `RUN_DB_PREPARE_ON_BOOT=true` を明示しない限り `db:prepare` を実行しないようにしています。
- **原因2: ヘルスチェックが重い経路を通る**
  - `/` はアプリ処理と DB 参照が発生するため、ヘルスチェック先として不利です。
  - `/up`（軽量ヘルスチェック）を追加済みです。Render の Health Check Path は **`/up`** を指定してください。
- **運用推奨**
  - マイグレーションは Web 起動時ではなく、Render の **Release Command**（または Build/Deploy フェーズ）で実行する。
  - Free プランのスリープ復帰遅延は完全には回避できないため、必要なら有料プラン（常時起動）を検討する。
