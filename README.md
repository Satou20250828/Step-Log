...
| 分類 | 使用技術 | 選定理由 |
|---|---|---|
| フロントエンド | HTML / CSS / JavaScript | 操作数を最小限に抑えた、軽快で直感的なUIを実装するため。複雑な状態管理や演出を必要としない設計のため、過剰なフレームワークは使用しない方針とした。 |
| バックエンド | Ruby / Ruby on Rails | 習慣登録・行動記録・履歴表示といったCRUD機能を少ないコード量で実装できるため。また、「1ユーザーにつき1習慣」「1日1記録」といった制約のある業務ロジックを明確に表現しやすいため。 |
| データベース | SQLite | 初期リリースでは大規模なデータ量や高負荷を想定しておらず、環境構築の負担を抑えて「まず完成させて公開する」ことを優先するため。 |
| 認証・ユーザー管理 | Cookie / LocalStorage | ログイン機能を排除し、初回アクセスからすぐに使える体験を提供するため。メールアドレスやパスワード入力による心理的ハードルをなくし、ブラウザ単位でユーザーを識別する設計とした。 |
| インフラ | Heroku または Render | Railsとの相性が良く、インフラ構築に時間をかけずにサービスを公開できるため。「作って終わり」ではなく、実際に利用できるURLを提供することを重視した。 |
